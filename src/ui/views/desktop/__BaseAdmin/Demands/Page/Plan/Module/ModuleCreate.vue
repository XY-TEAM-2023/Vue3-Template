<template>
  <div>
    <el-button type="primary" @click="onShow" size="small">
      {{ tryGetI18nText('DemandView.createModule') }}
    </el-button>

    <el-dialog v-model="display" :title="tryGetI18nText('DemandView.createModule')" width="600" :close-on-click-modal="false">
...
async function onSubmit() {
  ...
  await formDataRef.value.validate((valid) => {
    if (valid) {
      ...
      http_post('/api/admin/demand/page/plan/module/add', { plan_id: props.plan_id, ...formData.value }, true)
        .then(() => {
          display.value = false
          emit('refreshCb')
        })
