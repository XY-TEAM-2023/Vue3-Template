<template>
  <el-dialog v-model='display' :title="tryGetI18nText('DemandView.moduleSuccessEdit')" width='520' :close-on-click-modal='false'>
    <el-form-ex ref='formDataRef' :model='formData' label-position='left' :rules='formRules'>
      <el-form-item-label label='描述' v-model='formData.desc' prop='desc'  clearable />
    </el-form-ex>
...
async function onSubmit() {
  ...
  await formDataRef.value.validate((valid) => {
    if (valid) {
      ...
      http_post('/api/admin/demand/page/plan/module/edit/success', formData.value, true)
        .then(() => {
          display.value = false
          emit('refreshCb')
        })
