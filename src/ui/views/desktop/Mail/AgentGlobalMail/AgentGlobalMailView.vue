<template>
  ...
  >
    <el-table-column-integer label="AgentGlobalMailView.mailId" prop="id" align="center" width="100" />
    <el-table-column-label label="AgentGlobalMailView.title" prop="title" align="center" width="200" />
    <el-table-column-label label="AgentGlobalMailView.content" prop="content" align="center" can-edit @click="onDisplayContent" />
    <el-table-column-label label="AgentGlobalMailView.operator" prop="operator" align="center" width="100" />
    <el-table-column-timestamp label="AgentGlobalMailView.sendTime" prop="ts" align="center" width="160" />
  </el-table-ex>

  <el-dialog v-model="sendGlobalMail.display" :title="$t('AgentGlobalMailView.sendBtn')" width="40%" :close-on-click-modal="false">
    <el-form-ex label-position="left">
      <el-form-item-label label="AgentGlobalMailView.title" v-model="sendGlobalMail.data.title" maxlength="256" />
      <el-form-item-label
        label="AgentGlobalMailView.content"
...
const searchPageData = ref({
  page: 1,
  page_size: getDefaultPageSize('/api/admin//mail/global/agent/list'),
  total: 0,
})
...
function requestSearch() {
  ...
  http_post('/api/admin/mail/global/agent/list', { ...searchData.value, ...searchPageData.value }, false)
    .then((response) => {
...
function onSendGlobalMail() {
  ...
  sendGlobalMail.value.onSubmit = () => {
    ...
    http_post('/api/admin/mail/global/agent/send', sendGlobalMail.value.data, true)
      .then(() => {
        requestSearch()
